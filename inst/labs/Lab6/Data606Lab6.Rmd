...
The report is discussing polls taken from samples of the population and therefore the percentages are sample statistics. 
...
In order to generalize that reports findings, the samples would have to be random drawn from the representative population and be less than 10% of the total population. This report's samples meet the population criteria but there is no mention of being randomly sampled, so we can't know for certain if the findings can be generalized to the greater population.
...
The rows of table 6 correspond to the countries where people were polled. The rows of `atheism` correspond to sample respondents from all of the represented countries in alphabetical order.
...
• Verify the observations are independent and also verify the success-failure
condition using ˆp and n.

The sample size of 1002 for the US is less than 10% of the U.S. population and we can likely assume that the samples are randomly selected. Therefore the conditions for independence are met. The success-failure conditions require np >= 10 and n(1-p) >= 10.

```{r conditions, eval=TRUE}
n <- 1002
p <- 0.05
n*p
n*(1-p)
```
Both np and n(1-p) are both greater than 10.
...
```{r Italy-atheism, eval=TRUE}
it12 <- subset(atheism, nationality == "Italy" & year == "2012")
it12_atheist <- subset(it12, response == "atheist")
nrow(it12_atheist)/nrow(it12)
```

```{r Italy-conditions, eval=TRUE}
n <- 987
p <- 0.05
n*p
n*(1-p)
...
```{r it-atheism-ci, eval=TRUE, tidy = FALSE}
...
```{r ge-atheism, eval=TRUE}
ge12 <- subset(atheism, nationality == "Germany" & year == "2012")
ge12_atheist <- subset(ge12, response == "atheist")
nrow(ge12_atheist)/nrow(ge12)
```

```{r ge-conditions, eval=TRUE}
n <- 502
p <- 0.05
n*p
n*(1-p)
...
```{r ge-atheism-ci, eval=TRUE, tidy = FALSE}
...
As population proportion increases from 0 to 0.5, the margin of error also increases with the highest value at p=0.5. As population proportion continues to increase past 0.5 the reverse is true and the margin of error decreases down to 0 at p=1. Because margin of error is based on p∗(1−p), the two halves of the relationship - with p from 0 to 0.5 and from 0.5 to 1 - are mirror images of each other.
...
The sampling distribution is unimodal, mostly symmetrical, and centered around mean = 0.0997 with standard deviation of 0.0093 
...
The np calculation for Ecuador is less than 10 so it does not meet the conditions for inference.

```{r autstralia, eval=TRUE}
n <- 1040
p <- 0.1
n*p
n*(1-p)
...
The np and n(1-p) calculations for Australia is greater than 10 and does meet the conditions for inference. 
...
sp05_atheist <- subset(sp05, response == "atheist")

inference(sp05$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

The 2005 Spain 95 % Confidence interval = ( 0.083 , 0.1177 ).

```{r sp12, eval=TRUE, tidy = FALSE}

sp12 <- subset(atheism, nationality == "Spain" & year == "2012")
sp12_atheist <- subset(sp12, response == "atheist")

inference(sp12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
```

The 2012 Spain 95 % Confidence interval = ( 0.0734 , 0.1065 ).

The 2005 and 2012 confidence intervals do overlap and therefore there is not a convincing argument for change in Spain's atheism index.
...
us05_atheist <- subset(us05, response == "atheist")
...
These confidence intervals do not overlap and do show evidence of a change in the atheism index.
...
If we detect a change due to chance and reject a null hypothesis, that means we have made a Type I error. At a significance level of 0.05 for 39 countries, we would expect to make a Type I error with 39 * 0.05 = 1.95 or with 2 countries.
...
If the margin of error is 0.01, then at 95% confidence SE = 0.01/1.96. A 95% confidence level gives us p = 0.5. 
...
The sample must include at least 9,604 people. 
