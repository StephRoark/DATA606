...
```{r plot sample, eval=TRUE}
hist(samp)
```

The distribution is unimodal and somewhat right skewed and centered around 1500 sq ft which would be the typical house size.

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

Each sample will be different, with some being possibly very different, but most will have a similar shape as they are sampling the same population data.
...
It means that 95% of the time the estimate will be within 2 standard errors of the parameter. If the interval spreads out 2 standard errors from the point estimate, we can be roughly 95% confident that we have captured the true parameter.
...
Yes, the confidence interval does capture the population mean of the house size in Ames. It should capture this mean most of the time.
...
Aprroximately 95% of the intervals should capture the population mean.

##Loops Part 2

We need to create empty vectors where we can save the means and standard deviations that will be calculated from each sample. And while we're at it, let's also store the desired sample size as `n`.
...
The 95% confidence interval means that it will contain the population mean 95% of the time. The plot shows at least 1 mean that falls outside of the confidence interval. It could be different each time this interval is created as we are resampling the population, but one would expect that 95% of the time it will fall within the confidence intervals.
...
lower <- sum(upper_vector < pop_mean)
upper <- sum(lower_vector > pop_mean)
...
The 85% Confidence Interval has a -z^* score of 1.44.
