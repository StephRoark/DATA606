...
The study is an observation of the data collected from the survey. Observational studies cannot be used to show causation. The study could ask if there is a relationship or correlation between the beauty and the course evaluation results.
...
The distribution is left skewed with a median of 4.3. Most students gave higher scores with a few giving scores in the lower end which suggests that most students were satisfied and this is what we would expect.
...
```{r boxplot, eval=TRUE}
qplot(x = gender, y = age, data = evals, geom = "boxplot")

```

Female professors tend to be younger than male professors.
...
There seem to be some points that we cannot see.
...
Some points are plotted on top of each other because mutliple observations has the same scores.
...
y = 3.88 + 0.067 * x
where y is the score and x is the beauty average. The slope of 0.067 is relatively small and is not a factor but there is a small increase associated with beauty.
...
There residuals have constant variability with no apparent pattern, so the relationship appears to be linear.

The residuals are left skewed unimodal and curl away from the normal line.
...
The residuals have a left skew and deviate from the upper end of the normal probability line. 

 - the residuals of the model are nearly normal - the residuals are left skewed but normally shaped
 - the variability of the residuals is nearly constant - the variability does not greatly change between variables
 - the residuals are independent - the students samples are independent
 - each variable is linearly related to the outcome - there was a somewhat linear relationship but not exact

8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `gender` to the model changed the parameter estimate for `bty_avg`?
    
```{r bty_avg, eval=TRUE}   
lm(score ~ bty_avg + gender, data = evals, y = TRUE)
```

Yes, bty_avg while still small, is a factor in increasing the score and did get slight larger at 0.074.
...
\[
  \begin{aligned}
\widehat{score} &= 3.75 + 0.074 \times bty\_avg\end{aligned} + 0.1724 \times 1
\]
...
```{r scatter-score  - rank, eval = TRUE}
...
R treats each categorical variable separately by making equations for each level.
...
Number of professors would not seem to influence the model greatly.
...
The number of professors does indeed have the highest p-value.

cls_profssingle : p-value 0.77806

```{r output, eval=TRUE}
m_full$coefficients[11]
```

13. Interpret the coefficient associated with the ethnicity variable.

```{r effic, eval=TRUE}
m_full$coefficients[4]
```
The ethinicity variable increases the  score by .1235 when the professor is not a minority.
...
```{r back, eval=TRUE}
m_full_back <- lm(score ~  ethnicity + gender  + age + cls_perc_eval +  cls_credits + bty_avg + pic_color, data = evals)
summary(m_full_back)
```

score = 3.77 + 0.167 * ethnicity + 0.207 * gender - 0.206 * age + 0.005 * percent_eval + 0.505 * credits + 0.051 * bty_avg - 0.19  * pic_color 

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.

```{r back_resids, eval=TRUE}
plot(m_full_back$residuals)
hist(m_full_back$residuals)
plot(abs(m_full_back$residuals) ~ m_full_back$fitted.values)
#checking some of the other variables 
...
The residuals are fairly evenly spread out. The histogram shows that the model is nearly normal.
There are not a lot of outliers. The variables are independent.
...
As long as the sample is large enough and random, it should not have an effect. 
...
The factors which most influence a high evaluation score are gender, age and credits has the highest influence. So a young, male which teaches lower credit classes and also who is not a minority and is photogenic.

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?

The sample of student evaluators was below 10 and we do not know if the sampling of professor evaluations was randomly conducted. With a little more information possibly yes, but the student evaluator population is very limited and so therefor I would not be comfortable.